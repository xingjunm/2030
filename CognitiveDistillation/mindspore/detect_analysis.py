...
def extract_predictions(model, loader):
    """
    Extract predictions from model on loader
    """
    pred_list = []
    for images, _ in loader:
        # Convert to MindSpore tensor if needed
        if not isinstance(images, ms.Tensor):
            images = ms.Tensor(images)
        pred = model(images)
        # Get predictions (argmax)
        pred_indices = ms.ops.argmax(pred, axis=1)
        pred_list.append(pred_indices.asnumpy())
...
def main(exp):
    ...
    if train_filename is not None and test_filename is not None:
        ...
        if args.method == 'SS' or args.method == 'AC' or args.method == 'LID':
            # Flatten features
            train_results = train_results.reshape(train_results.shape[0], -1)
            test_results = test_results.reshape(test_results.shape[0], -1)
    ...
    if args.method == 'ABL':
        ...
        train_scores = 1 - min_max_normalization(train_results)
        test_scores = 1 - min_max_normalization(test_results)
    elif args.method == 'FCT':
        ...
        y_test_pred = extract_predictions(model, bd_test_loader)
        ...
        # LID not implemented yet
        raise NotImplementedError("LID analysis not implemented")
    elif 'CD' in args.method:
        detector = analysis.CognitiveDistillationAnalysis(od_type=args.method, norm_only=args.norm_only)
        # Convert numpy arrays to MindSpore tensors for CD analysis
        detector.train(ms.Tensor(train_results))
        train_scores = detector.analysis(ms.Tensor(train_results), is_test=False)
        test_scores = detector.analysis(ms.Tensor(test_results), is_test=True)
    else:
        raise TypeError('Unknown Method')
    ...
    if test_scores is not None:
        ...
        if args.method in ['AC', 'SS']:
            ...
            detector.train(ms.Tensor(train_results[train_clean_idx][:s]))
            y_pred = detector.predict(ms.Tensor(test_results), t=threshold)
