...
# Per mindspore-note.md #8: When loading PyTorch pre-trained models (.pt files),
# keep using PyTorch for loading and inference since these are fixed weights
...
class FrequencyAnalysis():
    def __init__(self, input_size=32):
        if input_size == 32:
            # Per mindspore-note.md #8: Keep using PyTorch to load pre-trained models
            self.clf = Detector().to(device)
            # Use absolute path or path relative to CognitiveDistillation root
            import os
            root_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
            ckpt_path = os.path.join(root_dir, 'checkpoints/frequency_detector_gtsrb_v2.pt')
            ckpt = torch.load(ckpt_path) # trained on gtsrb, use for CIFAR evaluation 
