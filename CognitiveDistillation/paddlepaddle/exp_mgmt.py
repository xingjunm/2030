...
class ExperimentManager():
    ...
    def save_state(self, target, name):
        if isinstance(target, paddle.DataParallel):
            target = target._layers
        filename = os.path.join(self.checkpoint_path, name) + '.pdparams'
        paddle.save(target.state_dict(), filename)
        if misc.get_rank() == 0:
