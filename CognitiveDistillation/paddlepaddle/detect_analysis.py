...
def extract_predictions(model, loader):
    ...
    for images, _ in loader:
        ...
        pred = model(images).argmax(axis=1)
        pred_list.append(pred)
    return paddle.concat(pred_list, axis=0)
...
def main(exp):
    ...
    if 'CIFAR10' in config.dataset.train_d_type:
        ...
        raise TypeError('Not Impelmented')
    ...
    if train_filename is not None and test_filename is not None:
        ...
        train_results = paddle.load(train_filename)
        test_results = paddle.load(test_filename)
        if args.method == 'SS' or args.method == 'AC' or args.method == 'LID':
            train_results = train_results.flatten(start_axis=1)
            test_results = test_results.flatten(start_axis=1)
    ...
    if args.method == 'ABL':
        ...
        train_scores = 1 - min_max_normalization(train_results).numpy()
        test_scores = 1 - min_max_normalization(test_results).numpy()
    elif args.method == 'FCT':
        # FCT already extracted the consistency score
        train_scores = train_results.numpy()
        test_scores = test_results.numpy()
    ...
    if test_scores is not None:
        ...
        if args.method in ['AC', 'SS']:
            ...
            detector.train(paddle.to_tensor(train_results[train_clean_idx][:s]))
            y_pred = detector.predict(test_results, t=threshold)
