...
def epoch_exp_stats():
    ...
    for images, labels in no_shuffle_loader:
        images = paddle.to_tensor(images.numpy() if hasattr(images, 'numpy') else images)
        labels = paddle.to_tensor(labels.numpy() if hasattr(labels, 'numpy') else labels)
        logits = model(images)
        loss = F.cross_entropy(logits, labels, reduction='none')
        _, predicted = paddle.max(logits, axis=1), paddle.argmax(logits, axis=1)
...
def train(epoch):
    ...
    # Track exp stats
    if isinstance(criterion, paddle.nn.CrossEntropyLoss):
...
def main():
    ...
    # Prepare Model
    model = config.model()
    optimizer = config.optimizer(parameters=model.parameters())
