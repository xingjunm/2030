...
class Generator(nn.Sequential):
    def __init__(self, opt, out_channels=None):
        ...
        for step in range(steps):
            self.add_sublayer("convblock_down_{}".format(2 * step), Conv2dBlock(channel_current, channel_next))
            self.add_sublayer("convblock_down_{}".format(2 * step + 1), Conv2dBlock(channel_next, channel_next))
            self.add_sublayer("downsample_{}".format(step), DownSampleBlock())
            if step < steps - 1:
                channel_current = channel_next
                channel_next *= 2

        self.add_sublayer("convblock_middle", Conv2dBlock(channel_next, channel_next))

        channel_current = channel_next
        channel_next = channel_current // 2
        for step in range(steps):
            self.add_sublayer("upsample_{}".format(step), UpSampleBlock())
            self.add_sublayer("convblock_up_{}".format(2 * step), Conv2dBlock(channel_current, channel_current))
            if step == steps - 1:
                self.add_sublayer(
                    "convblock_up_{}".format(2 * step + 1), Conv2dBlock(channel_current, channel_next, relu=False)
                )
            else:
                self.add_sublayer("convblock_up_{}".format(2 * step + 1), Conv2dBlock(channel_current, channel_next))
            channel_current = channel_next
