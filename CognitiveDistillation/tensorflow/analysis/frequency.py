...
# Note: According to tensorflow-note.md item 8, we keep using PyTorch for loading
# and running the pretrained model since it's a fixed pretrained checkpoint.
# We only convert outputs to numpy arrays at the interface boundaries.
...
class ConvBrunch(nn.Module):
    """Keep PyTorch implementation for pretrained model compatibility"""
...
class FrequencyAnalysis():
    def __init__(self, input_size=32):
        if input_size == 32:
            # Keep using PyTorch for loading pretrained model (tensorflow-note.md item 8)
    ...
    def predict(self, data, t=1):
        ...
        with torch.no_grad():
            for i in tqdm(range(len(data))):
                x, _ = data[i]
                # Handle both torch.Tensor and tf.Tensor inputs
                if hasattr(x, 'numpy'):
