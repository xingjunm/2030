...
class DatasetGenerator():
    def __init__(self, train_bs=128, eval_bs=256, seed=0, n_workers=4,
        ...
        if train_d_type not in transform_options:
            raise ValueError('Unknown Dataset')  # Fixed: use ValueError instead of bare raise
    ...
    def get_loader(self, train_shuffle=True):
        # Using paddle.io.DataLoader instead of torch.utils.data.DataLoader
        # Note: PaddlePaddle DataLoader doesn't have pin_memory parameter
        # According to paddlepaddle-note.md #4, PaddlePaddle's DataLoader 
        # will auto-convert numpy arrays to paddle.Tensor
