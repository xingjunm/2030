...
class Attacker():
    def __init__(self, model, epsilon=8./255., v_min=0., v_max=1.,
        ...
        # autopgd_pt and fab_pt don't need device parameter in PaddlePaddle version
        ...
        if version == 'MD':
            ...
            raise TypeError('Unknown')  # Using TypeError to match original behavior
    ...
    def evaluate(self):
        ...
        for images, labels in self.data_loader:
            # In PaddlePaddle, tensors are automatically placed on the appropriate device
            # Converting from numpy arrays if needed (following note #4)
            if isinstance(images, np.ndarray):
                images = paddle.to_tensor(images)
            if isinstance(labels, np.ndarray):
                labels = paddle.to_tensor(labels)
            
            nc = paddle.zeros_like(labels)
            ...
            # Using argmax instead of max(1)[1] for PaddlePaddle
            clean_pred = paddle.argmax(clean_logits, axis=1).detach()
            clean_correct = paddle.sum(clean_pred == labels).item()
            ...
            # Using argmax instead of max(1)[1] for PaddlePaddle
            adv_pred = paddle.argmax(adv_logits, axis=1).detach()
            adv_correct = paddle.sum(adv_pred == labels).item()
