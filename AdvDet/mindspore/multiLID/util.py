...
def multiLID(model, X, X_noisy, X_adv, dataset, k=10, batch_size=100):
    ...
    def estimate(i_batch):
        ...
        # RISK_INFO: [张量操作差异] - Tensor is initialized with zeros via np.zeros, whereas the original PyTorch code creates an uninitialized tensor.
        batch = Tensor(np.zeros((n_feed, shape[0], shape[1], shape[2]), dtype=np.float32), mindspore.float32)
        # RISK_INFO: [张量操作差异] - Tensor is initialized with zeros via np.zeros, whereas the original PyTorch code creates an uninitialized tensor.
        batch_adv = Tensor(np.zeros((n_feed, shape[0], shape[1], shape[2]), dtype=np.float32), mindspore.float32)
        # RISK_INFO: [张量操作差异] - Tensor is initialized with zeros via np.zeros, whereas the original PyTorch code creates an uninitialized tensor.
        batch_noisy = Tensor(np.zeros((n_feed, shape[0], shape[1], shape[2]), dtype=np.float32), mindspore.float32)
        
        for j in range(n_feed):
            batch[j,:,:,:] = Tensor(X[start + j].astype(np.float32), mindspore.float32)
            batch_adv[j,:,:,:] = Tensor(X_adv[start + j].astype(np.float32), mindspore.float32)
            batch_noisy[j,:,:,:] = Tensor(X_noisy[start + j].astype(np.float32), mindspore.float32)
        ...
        for i, func in enumerate(funcs):
            if i+1 == lid_dim:
                func = model
            X_act = func(batch).asnumpy()
            X_act = X_act.reshape((n_feed, -1))

            X_adv_act = func(batch_adv).asnumpy()
            X_adv_act = X_adv_act.reshape((n_feed, -1))

            X_noisy_act = func(batch_noisy).asnumpy()
