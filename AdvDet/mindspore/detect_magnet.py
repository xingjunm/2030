...
def test(dic, X, thrs):
    ...
    pred_labels = []
    for d in dic:
        # RISK_INFO: [API 行为不等价] - mindspore.Tensor() 可能会创建数据副本, 而 torch.as_tensor() 尝试共享内存, 存在潜在行为差异。
        m = dic[d].mark(Tensor(X))
        dist_all.append(m)
...
def main(args):
    ...
    if args.dataset != 'imagenet':
        assert os.path.isfile('{}cnn_{}.ckpt'.format(checkpoints_dir, args.dataset)), \
    ...
    if args.dataset == 'mnist':
        ...
        model_class = myModel(mode='load', filename='cnn_{}.ckpt'.format(args.dataset))
        ...
        model_class = myModel(filename='cnn_{}.ckpt'.format(args.dataset))
        classifier = model_class.classifier
        clip_min, clip_max = 0,1
        v_noise=0.025
        p1=1
    ...
    detector_i_filename = '{}_magnet_detector_i.ckpt'.format(args.dataset)
    detector_ii_filename = '{}_magnet_detector_ii.ckpt'.format(args.dataset)
    ...
    # Create MindSpore dataset using the loader
    def create_dataset_from_loader(loader, batch_size=256, is_train=True):
        def generator():
            for i in range(len(loader)):
                yield loader[i]
        
        dataset = ds.GeneratorDataset(generator, ["data", "label"], shuffle=is_train)
        dataset = dataset.batch(batch_size, drop_remainder=False)
        return dataset
    
    datas = create_dataset_from_loader(loader_train, batch_size=256, is_train=True)
    ...
    # RISK_INFO: [API 行为不等价] - mindspore.Tensor() 可能会创建数据副本, 而 torch.as_tensor() 尝试共享内存, 存在潜在行为差异。
    operator = Operator(Tensor(x_val), Tensor(X_test), Tensor(Y_test), classifier.model, detector_dict, reformer)
    ...
    for attack in ATTACKS:
        ...
        # RISK_INFO: [API 行为不等价] - mindspore.Tensor() 可能会创建数据副本, 而 torch.as_tensor() 尝试共享内存, 存在潜在行为差异。
        testAttack = AttackData(Tensor(X_test_adv), Tensor(np.argmax(Y_test, axis=1)), attack)
