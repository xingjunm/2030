...
def batch(curr_model, data, batch_size):
    ...
    for i in tqdm(range(num_batches)):
        ...
        batch = Tensor(data[start_idx:end_idx], dtype=mstype.float32)
        l_out_batch = curr_model(batch).asnumpy()
...
def main(args):
    ...
    n_layers = len([i for i in classifier.model.cells_and_names()])
    ...
    for l_indx in range(start_indx, n_layers):
        ...
        if not os.path.isfile(layer_data_path):
            ...
            if l_indx+1 == n_layers:
                ...
                cells = list(classifier.model.cells())
                curr_model = nn.SequentialCell(cells[:l_indx+1])
            curr_model.set_train(False)
    ...
    for l_indx in range(start_indx, n_layers-1):
        ...
        if not os.path.isfile(model_path):
            if os.path.isfile(layer_data_path_current) & os.path.isfile(layer_data_path_next):
                ...
                layer_data_next = np.load(layer_data_next)
                ...
                layer_data_current = Tensor(layer_data_current, dtype=mstype.float32)
                layer_data_next = Tensor(layer_data_next, dtype=mstype.float32)
                vi_current = model_current(layer_data_current)
                vi_next = model_next(layer_data_next)
                vi_current = vi_current.asnumpy()
                vi_next = vi_next.asnumpy()
        ...
        for l_indx in range(start_indx, n_layers-1):
            ...
            if not os.path.isfile(vi_decision_adv_path):
                if os.path.isfile(layer_adv_path_current) & os.path.isfile(layer_adv_path_next) & os.path.isfile(model_path):
                    ...
                    layer_data_current = Tensor(layer_data_current, dtype=mstype.float32)
                    layer_data_next = Tensor(layer_data_next, dtype=mstype.float32)
                    vi_current = model_current(layer_data_current).asnumpy()
                    vi_next = model_next(layer_data_next).asnumpy()
