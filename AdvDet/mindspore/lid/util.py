...
def mle_single(data, x, k=20):
    ...
    # Calculate cdist manually in MindSpore
    a = ops.cdist(Tensor(x, mindspore.float32), Tensor(data, mindspore.float32)).asnumpy()
...
def mle_batch(data, batch, k):
    k = min(k, len(data)-1)
    f = lambda v: - k / ops.sum(ops.log(v/v[-1]))
    a = ops.cdist(batch, data)
    # RISK_INFO: [API 行为不等价] - The underlying sorting algorithms of torch.sort and ops.sort are not guaranteed to be identical (e.g., stable vs. unstable sort). This may lead to different results if there are duplicate distance values.
    a = ops.sort(a, axis=1)[0]
    a = a[:,1:k+1]
    lid = ops.stack([f(x) for x in ops.unstack(a, axis=0)]).asnumpy()
...
def get_lids_random_batch(model, X, X_noisy, X_adv, dataset, k=10, batch_size=100):
    ...
    funcs = [nn.SequentialCell(*list(model.cells())[:i+1]) for i in range(len(list(model.cells())))]
    lid_dim = len(funcs)
    ...
    def estimate(i_batch):
        ...
        for i, func in enumerate(funcs):
            if i+1 == lid_dim:
                func = model
            X_act = func(Tensor(X[start:end], mindspore.float32))
            X_act = ops.reshape(X_act, (int(n_feed), -1))

            X_adv_act = func(Tensor(X_adv[start:end], mindspore.float32))
            X_adv_act = ops.reshape(X_adv_act, (int(n_feed), -1))

            X_noisy_act = func(Tensor(X_noisy[start:end], mindspore.float32))
            X_noisy_act = ops.reshape(X_noisy_act, (int(n_feed), -1))
