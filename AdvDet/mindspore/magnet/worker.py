...
class AEDetector:
    ...
    def mark(self, X):
        X_tensor = X if isinstance(X, Tensor) else Tensor(X, dtype=mindspore.float32)
        X_output = self.model(X_tensor)
        diff = ops.abs(X_tensor - X_output).asnumpy()
...
class SimpleReformer:
    ...
    def heal(self, X):
        X = self.model(X)
        return ops.clip_by_value(X, clip_value_min=0.0, clip_value_max=1.0)
...
class DBDetector:
    ...
    def mark_jsd(self, X):
        ...
        # Convert MindSpore tensors to numpy arrays for JSD calculation
        Pp_np = Pp.asnumpy()
        Pr_np = Pr.asnumpy()
        marks = [(JSD(Pp_np[i], Pr_np[i])) for i in range(len(Pr_np))]
...
class Operator:
    def __init__(self, x_val, x_test, y_test, classifier, det_dict, reformer):
        ...
        self.normal = self.operate(AttackData(self.x_test, ops.argmax(self.y_test, axis=1).asnumpy(), "Normal"))
    ...
    def operate(self, untrusted_obj):
        ...
        if not isinstance(Y_true, Tensor):
            Y_true = Tensor(Y_true, dtype=mindspore.int32)
        
        X_prime = self.reformer.heal(X)
        
        # Y = torch.argmax(self.classifier(X), axis=1)
        Y = ops.concat(self.batch(X, batch_size=4096), axis=0)
        Y_judgement = (Y == Y_true[:len(X_prime)]).asnumpy()
        # Y_prime = torch.argmax(self.classifier(X_prime), axis=1)
        Y_prime = ops.concat(self.batch(X_prime, batch_size=4096), axis=0)
        Y_prime_judgement = (Y_prime == Y_true[:len(X_prime)]).asnumpy()
    ...
    def batch(self, X, batch_size=128):
        ...
        for i in range(0, X.shape[0], batch_size):
            ...
            batch_X = X[i:end_idx]
            batch_predictions = ops.argmax(self.classifier(batch_X), axis=1)
