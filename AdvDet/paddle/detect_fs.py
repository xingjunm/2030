...
def get_distance(model, dataset, X1):
    # RISK_INFO: [API 行为不等价] - The `model.predict()` method is framework-dependent. Its output (e.g., logits vs. probabilities, numerical precision) may differ between the original and new implementations, potentially altering downstream logic.
...
def main(args):
    ...
    if args.dataset != 'imagenet':
        # [必要的修改] PaddlePaddle uses .pdparams extension instead of PyTorch's .pt
        assert os.path.isfile('{}cnn_{}.pdparams'.format(checkpoints_dir, args.dataset)), \
    ...
    if args.dataset == 'mnist':
        ...
        # [必要的修改] PaddlePaddle uses .pdparams extension instead of PyTorch's .pt
        model_class = myModel(mode='load', filename='cnn_{}.pdparams'.format(args.dataset))
        ...
        # [必要的修改] PaddlePaddle uses .pdparams extension instead of PyTorch's .pt
        model_class = myModel(filename='cnn_{}.pdparams'.format(args.dataset))
        classifier = model_class.classifier
    ...
    # RISK_INFO: [API 行为不等价] - The `classifier.predict()` method is framework-dependent. Its output may differ between the original and new implementations, potentially altering which samples are considered 'correctly classified'.
    ...
    # RISK_INFO: [随机性处理差异] - Reproducibility of `random.sample` depends on `set_seed()` also seeding Python's built-in `random` module via `random.seed()`. This is a common oversight in framework migrations.
    ...
    for attack in ATTACKS:
        ...
        print(f"DEBUG: Loaded X_test_adv shape: {X_test_adv.shape}")
        X_test_adv = reduce_precision_py(X_test_adv, 256)
        print(f"DEBUG: After reduce_precision_py shape: {X_test_adv.shape}")
    
        print(f"DEBUG: inds_correct length: {len(inds_correct)}, max: {inds_correct.max() if len(inds_correct) > 0 else 'N/A'}")
        X_test_adv = X_test_adv[inds_correct]
        print(f"DEBUG: After inds_correct shape: {X_test_adv.shape}")
        X_test_adv = X_test_adv[indx_test]
        print(f"DEBUG: After indx_test shape: {X_test_adv.shape}")
        
        # loss, acc_suc = model.evaluate(X_test_adv, y_test, verbose=0)
        # RISK_INFO: [API 行为不等价] - The `classifier.predict()` method is framework-dependent. Its output may differ between the original and new implementations, potentially altering adversarial success rate calculation.
