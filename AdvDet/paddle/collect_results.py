"""
Collect and aggregate results from all detection methods for PaddlePaddle
"""
import matplotlib
        ...
        for csv_dir in CSVs_dir:
            ...
            if os.path.isfile(csv_file):
                with open(csv_file, 'r') as file:
                    ...
                    # CRITICAL_ERROR: [算法流程改变] - 新增了 `if len(current_result) >= 3:` 条件判断，改变了原有的控制流。原代码在结果行数不足时会直接报错，而新代码会跳过处理。
                    # 不一致的实现:
                    ...
                    # all[key_dr] = DR
                    # all[key_fpr] = AUC
                    raise NotImplementedError("新增了 `if len(current_result) >= 3:` 条件判断，改变了原有的控制流。原代码在结果行数不足时会直接报错，而新代码会跳过处理。")
