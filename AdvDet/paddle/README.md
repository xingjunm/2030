## An open-source toolkit for Adversarial Example Detection (AED) - PaddlePaddle Version
This repo implements a set of detection methods for adversarial example detection (AED) using PaddlePaddle framework. It supports experiments on MNIST, CIFAR-10, and SVHN datasets with 7 detection methods: KDE, LID, NSS, FS, Magnet, NIC, and MultiLID. This is a PaddlePaddle port of the original PyTorch implementation.
...
To generate adversarial examples, run `generate_adv.py -d=<dataset>`. After running the program, adversarial examples will be automatically generated and saved for subsequent detection. The perturbation for $L_{\infty}$ are `epsilons = [8/256, 16/256, 32/256, 64/256, 80/256, 128/256]`, for L1 are `epsilons1 = [5, 10, 15, 20, 25, 30, 40]`, and for L2 are `epsilons2 = [0.125, 0.25, 0.3125, 0.5, 1, 1.5, 2]`.
...
Note: This PaddlePaddle implementation aims to reproduce the results from the original PyTorch version. Minor differences in results may occur due to framework-specific implementations.

### Framework Differences
This implementation has been ported from PyTorch to PaddlePaddle with the following key changes:
- `torch` → `paddle`
- `nn.Module` → `nn.Layer`
- `Conv2d` → `Conv2D`, `BatchNorm2d` → `BatchNorm2D`
- `view()` → `reshape()`
- Device handling: `'cuda'` → `'gpu'` with `paddle.set_device()`
