"""
LID (Local Intrinsic Dimensionality) detection method for PaddlePaddle
Reference: Ma et al. "Characterizing Adversarial Subspaces Using Local Intrinsic Dimensionality" (2018)
"""
import argparse
import numpy as np
import os
...
from lid.util import (random_split, block_split, train_lr, compute_roc, 
                      get_lids_random_batch, get_noisy_samples)
...
def get_lid(model, X_test, X_test_noisy, X_test_adv, k=10, batch_size=100, dataset='mnist'):
    ...
    :param model: 
    :param X_test: clean test samples
    :param X_test_noisy: noisy test samples
    :param X_test_adv: adversarial test samples
    ...
    lids_normal, lids_noisy, lids_adv = get_lids_random_batch(
        model, X_test, X_test_noisy, X_test_adv, dataset, k, batch_size
    )
...
def main(args):
    ...
    if os.path.isfile(lid_file_X) & os.path.isfile(lid_file_Y):
        print("Loading cached LID features...")
        X = np.load(lid_file_X)
        Y = np.load(lid_file_Y)
    else:
        print("Computing LID features...")
