...
def main(args):
    ...
    if args.dataset == 'mnist':
        ...
        model_mnist = model(mode='train', filename='cnn_{}.pdparams'.format(args.dataset), epochs=args.epochs, batch_size=args.batch_size)
    elif args.dataset == 'cifar':
        from baseline.cnn.cnn_cifar10 import CIFAR10CNN as model
        model_cifar = model(mode='train', filename='cnn_{}.pdparams'.format(args.dataset), epochs=args.epochs, batch_size=args.batch_size)
    elif args.dataset == 'svhn':
        from baseline.cnn.cnn_svhn import SVHNCNN as model
        model_svhn = model(mode='train', filename='cnn_{}.pdparams'.format(args.dataset), epochs=args.epochs, batch_size=args.batch_size)
