...
def test(dic, X, thrs):
    ...
    pred_labels = []
    for d in dic:
        # RISK_INFO: [API 行为不等价] - paddle.to_tensor() 总是复制数据，而 torch.as_tensor() 在可能的情况下共享内存。这可能导致行为差异，尽管在此上下文中风险较低。
        m = dic[d].mark(paddle.to_tensor(X))
        dist_all.append(m)
...
def main(args):
    ...
    # [必要的修改] PaddlePaddle uses .pdparams extension instead of PyTorch's .pt
    detector_i_filename = '{}_magnet_detector_i.pdparams'.format(args.dataset)
    detector_ii_filename = '{}_magnet_detector_ii.pdparams'.format(args.dataset)
    ...
    # RISK_INFO: [API 行为不等价] - PaddlePaddle的 DataLoader 和 PyTorch 的 DataLoader 在随机数生成和多进程处理上可能存在细微差异，即使种子相同，也可能影响训练数据的顺序。
    ...
    # RISK_INFO: [API 行为不等价] - paddle.to_tensor() 总是复制数据，而 torch.as_tensor() 在可能的情况下共享内存。这可能导致行为差异，尽管在此上下文中风险较低。
    operator = Operator(paddle.to_tensor(x_val), paddle.to_tensor(X_test), paddle.to_tensor(Y_test), classifier.model, detector_dict, reformer)
    ...
    for attack in ATTACKS:
        ...
        # RISK_INFO: [API 行为不等价] - paddle.to_tensor() 总是复制数据，而 torch.as_tensor() 在可能的情况下共享内存。这可能导致行为差异，尽管在此上下文中风险较低。
        testAttack = AttackData(paddle.to_tensor(X_test_adv), paddle.to_tensor(np.argmax(Y_test, axis=1), dtype='int64'), attack)
