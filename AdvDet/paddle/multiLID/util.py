...
def multiLID(model, X, X_noisy, X_adv, dataset, k=10, batch_size=100):
    ...
    model.eval()
    funcs = [nn.Sequential(*list(model.children())[:i+1]) for i in range(len(list(model.children())))]
    lid_dim = len(funcs)
    ...
    def estimate(i_batch):
        ...
        batch = paddle.zeros([n_feed, shape[0], shape[1], shape[2]], dtype='float32')
        batch_adv = paddle.zeros([n_feed, shape[0], shape[1], shape[2]], dtype='float32')
        batch_noisy = paddle.zeros([n_feed, shape[0], shape[1], shape[2]], dtype='float32')
        
        for j in range(n_feed):
            batch[j,:,:,:] = paddle.to_tensor(X[start + j], dtype='float32')
            batch_adv[j,:,:,:] = paddle.to_tensor(X_adv[start + j], dtype='float32')
            batch_noisy[j,:,:,:] = paddle.to_tensor(X_noisy[start + j], dtype='float32')
        ...
        for i, func in enumerate(funcs):
            if i+1 == lid_dim:
                func = model
            X_act = func(batch).detach().numpy()
            X_act = X_act.reshape((n_feed, -1))

            X_adv_act = func(batch_adv).detach().numpy()
            X_adv_act = X_adv_act.reshape((n_feed, -1))

            X_noisy_act = func(batch_noisy).detach().numpy()
