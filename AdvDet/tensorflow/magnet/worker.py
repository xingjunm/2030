...
class AEDetector:
    ...
    def mark(self, X):
        ...
        # RISK_INFO: [API 行为不等价] - tf.Tensor.numpy() 与 torch.Tensor.cpu().detach().numpy() 在处理设备间数据传输时可能存在细微差异。
        diff = tf.abs(X - self.model(X)).numpy()
...
class SimpleReformer:
    ...
    def heal(self, X):
        X = self.model(X)
        # RISK_INFO: [API 行为不等价] - tf.clip_by_value 和 torch.clip 在处理NaN和无穷大值时可能存在细微差异。
...
class Classifier:
    ...
    def classify(self, X, option="logit", T=1):
        # RISK_INFO: [API 行为不等价] - TensorFlow模型调用可能不会像PyTorch那样自动处理数据格式（channels-first/last），如果模型期望特定格式，此处可能与AEDetector中的处理不一致。
        if option == "logit":
            return self.model(X)
        if option == "prob":
            logits = self.model(X)/T
            # RISK_INFO: [API 行为不等价] - tf.nn.softmax 和 torch.softmax 在数值稳定性实现上可能存在差异。
...
class Operator:
    ...
    def operate(self, untrusted_obj):
        ...
        # Y = tf.argmax(self.classifier(X), axis=1)
        Y = tf.concat(self.batch(X, batch_size=4096), axis=0)
        # RISK_INFO: [API 行为不等价] - .numpy() 与 .cpu().detach().numpy() 的行为可能存在差异，尤其是在异步GPU执行时。
        Y_judgement = (Y == Y_true[:len(X_prime)]).numpy()
        # Y_prime = tf.argmax(self.classifier(X_prime), axis=1)
        Y_prime = tf.concat(self.batch(X_prime, batch_size=4096), axis=0)
        Y_prime_judgement = (Y_prime == Y_true[:len(X_prime)]).numpy()
    ...
    def batch(self, X, batch_size=128):
        ...
        """
        predictions = []
        # RISK_INFO: [API 行为不等价] - X.shape[0] 在TensorFlow中等价于PyTorch的X.size(0)，但前提是X是Tensor。
        for i in range(0, X.shape[0], batch_size):
            end_idx = min(i+batch_size, X.shape[0])
            batch_X = X[i:end_idx]
            # RISK_INFO: [张量操作差异] - tf.argmax 和 torch.argmax 在处理多个最大值时返回的索引可能不同，这会影响可复现性。
            batch_predictions = tf.argmax(self.classifier(batch_X), axis=1)
