...
def dense(input_shape):
    model = keras.Sequential([
        layers.Dense(10, input_shape=(input_shape[1],)),
        layers.Softmax()
    ])
    criterion = keras.losses.CategoricalCrossentropy()
    # RISK_INFO: [API 行为不等价] - tf.keras.optimizers.Adam's default epsilon is 1e-7, while torch.optim.Adam's default is 1e-8. This may lead to different optimization behavior and numerical stability.
...
def batch(curr_model, data, batch_size):
    ...
    for i in tqdm(range(num_batches)):
        ...
        # RISK_INFO: [张量操作差异] - The original PyTorch code uses .cpu().detach().numpy(). The .detach() call explicitly removes the tensor from the computation graph. While TensorFlow's .numpy() in eager mode achieves a similar result by copying data to the CPU and breaking the gradient tape, this implicit behavior is a difference in implementation.
        l_out_batch = curr_model(batch).numpy()
...
def main(args):
    ...
    if args.dataset == 'mnist':
        ...
        model_class = myModel(filename='cnn_{}.h5'.format(args.dataset))
        classifier = model_class.classifier
        start_indx = 1
    ...
    #-----------------------------------------------# 
    # Build the model if it hasn't been built yet (needed for loaded models)
    if not classifier.model.built:
        # Build with dummy input to infer shapes
        dummy_input = tf.zeros((1,) + X_train.shape[1:])
        _ = classifier.model(dummy_input)
    
    # Get logical block indices instead of iterating through all layers
    block_indices = get_logical_block_indices(classifier.model)
    n_layers = len(block_indices)
    projectors = {}  # Store PCA projectors for each layer

    #for train
    for l_idx, l_indx in enumerate(block_indices[start_indx:], start=start_indx):
        layer_data_path = '{}{}_{}_normal.npy'.format(nic_layers_dir, args.dataset, l_idx)
        if not os.path.isfile(layer_data_path):

            if l_idx == n_layers - 1:
                curr_model = classifier.model
            else:
                # Create a model up to the current block
                # For Sequential models, we need to build a new model with subset of layers
                layers_subset = classifier.model.layers[:l_indx+1]
                curr_model = keras.Sequential(layers_subset)
            ...
            if l_out.shape[1]>5000:
                ...
                # Save the fitted projector
                projector_path = '{}{}_{}_projector.pkl'.format(nic_layers_dir, args.dataset, l_idx)
                with open(projector_path, 'wb') as f:
                    pickle.dump(projector, f)
                projectors[l_idx] = projector
            else:
                np.save(layer_data_path, l_out)
                projectors[l_idx] = None
            print(layer_data_path)
    
        #for test
        layer_data_path = '{}{}_{}_test.npy'.format(nic_layers_dir, args.dataset, l_idx)
        ...
        if not os.path.isfile(layer_data_path):
            ...
            if l_out.shape[1]>5000:
                # Load or get the projector for this layer
                if l_idx not in projectors:
                    projector_path = '{}{}_{}_projector.pkl'.format(nic_layers_dir, args.dataset, l_idx)
                    if os.path.isfile(projector_path):
                        with open(projector_path, 'rb') as f:
                            projectors[l_idx] = pickle.load(f)
                    else:
                        # If projector doesn't exist, skip PCA reduction
                        print(f"Warning: PCA projector not found for layer {l_idx}, saving full features")
                        np.save(layer_data_path, l_out)
                        continue
                
                projector = projectors[l_idx]
        ...
        for attack in ATTACKS:
            ...
            layer_data_path = '{}{}_{}_{}.npy'.format(nic_layers_dir, args.dataset, l_idx, attack)
            if not os.path.isfile(layer_data_path):
    ...
    #-----------------------------------------------# 
    min_features = 5000
    for l_idx in range(start_indx, n_layers):
        layer_data_path = '{}{}_{}_normal.npy'.format(nic_layers_dir, args.dataset, l_idx)
        model_path = '{}{}_{}_pi.model'.format(nic_layers_dir, args.dataset, l_idx)
        pi_predict_normal_path = '{}{}_{}_pi_predict_normal.npy'.format(nic_layers_dir, args.dataset, l_idx)
        pi_decision_normal_path = '{}{}_{}_pi_decision_normal.npy'.format(nic_layers_dir, args.dataset, l_idx)
        if not os.path.isfile(model_path):
            if os.path.isfile(layer_data_path):
                ...
                et = time.time()
                t=round((et-st)/60, 2)
                print('Training PI on {}, layer {} is completed on {} min(s).'.format(args.dataset, l_idx, t))

    #-----------------------------------------------#
    #                  Train VIs                    #
    #-----------------------------------------------# 
    for l_idx in range(start_indx, n_layers-1):
        layer_data_path_current = '{}{}_{}_normal.npy'.format(nic_layers_dir, args.dataset, l_idx)
        layer_data_path_next = '{}{}_{}_normal.npy'.format(nic_layers_dir, args.dataset, l_idx+1)
        model_path = '{}{}_{}_vi.model'.format(nic_layers_dir, args.dataset, l_idx)
        vi_train_path = '{}{}_{}_vi_train.npy'.format(nic_layers_dir, args.dataset, l_idx)
        vi_predict_normal_path = '{}{}_{}_vi_predict_normal.npy'.format(nic_layers_dir, args.dataset, l_idx)
        vi_decision_normal_path = '{}{}_{}_vi_decision_normal.npy'.format(nic_layers_dir, args.dataset, l_idx)
        if not os.path.isfile(model_path):
            if os.path.isfile(layer_data_path_current) & os.path.isfile(layer_data_path_next):
                ...
                layer_data_current = tf.constant(layer_data_current, dtype=tf.float32)
                layer_data_next = tf.constant(layer_data_next, dtype=tf.float32)
                vi_current = model_current(layer_data_current)
                vi_next = model_next(layer_data_next)
                # RISK_INFO: [张量操作差异] - The original PyTorch code uses .detach().numpy(). The .detach() call explicitly removes the tensor from the computation graph. This TensorFlow implementation relies on the implicit behavior of .numpy() to break the gradient tape, which is a difference in implementation.
                vi_current = vi_current.numpy()
                # RISK_INFO: [张量操作差异] - The original PyTorch code uses .detach().numpy(). The .detach() call explicitly removes the tensor from the computation graph. This TensorFlow implementation relies on the implicit behavior of .numpy() to break the gradient tape, which is a difference in implementation.
                vi_next = vi_next.numpy()
                ...
                et = time.time()
                t=round((et-st)/60, 2)
                print('Training VI on {}, layer {} is completed on {} min(s).'.format(args.dataset, l_idx, t))
    ...
    for attack in ATTACKS:
        ...
        pis = np.array([])
        for l_idx in range(start_indx, n_layers):
            layer_adv_path = '{}{}_{}_{}.npy'.format(nic_layers_dir, args.dataset, l_idx, attack)
            model_path = '{}{}_{}_pi.model'.format(nic_layers_dir, args.dataset, l_idx)
            pi_decision_adv_path = '{}{}_{}_pi_decision_{}.npy'.format(nic_layers_dir, args.dataset, l_idx, attack)
        ...
        vis = np.array([])
        for l_idx in range(start_indx, n_layers-1):
            layer_adv_path_current = '{}{}_{}_{}.npy'.format(nic_layers_dir, args.dataset, l_idx, attack)
            layer_adv_path_next = '{}{}_{}_{}.npy'.format(nic_layers_dir, args.dataset, l_idx+1, attack)
            model_path = '{}{}_{}_vi.model'.format(nic_layers_dir, args.dataset, l_idx)
            vi_decision_adv_path = '{}{}_{}_vi_decision_{}.npy'.format(nic_layers_dir, args.dataset, l_idx, attack)
            if not os.path.isfile(vi_decision_adv_path):
                if os.path.isfile(layer_adv_path_current) & os.path.isfile(layer_adv_path_next) & os.path.isfile(model_path):
                    ...
                    layer_data_next = tf.constant(layer_data_next, dtype=tf.float32)
                    # RISK_INFO: [张量操作差异] - The original PyTorch code uses .detach().numpy(). The .detach() call explicitly removes the tensor from the computation graph. This TensorFlow implementation relies on the implicit behavior of .numpy() to break the gradient tape, which is a difference in implementation.
                    vi_current = model_current(layer_data_current).numpy()
                    # RISK_INFO: [张量操作差异] - The original PyTorch code uses .detach().numpy(). The .detach() call explicitly removes the tensor from the computation graph. This TensorFlow implementation relies on the implicit behavior of .numpy() to break the gradient tape, which is a difference in implementation.
                    vi_next = model_next(layer_data_next).numpy()
