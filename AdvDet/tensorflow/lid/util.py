...
# RISK_INFO: [张量操作差异] - 该函数从 PyTorch+NumPy 迁移到纯 TensorFlow。`torch.cdist` 被 `tf.norm` 手动实现替代，排序和聚合操作也改为 TensorFlow API。虽然逻辑上等价，但底层实现和数值精度可能存在差异。
def mle_single(data, x, k=20):
    ...
    # TensorFlow cdist computation
    data_tf = tf.convert_to_tensor(data, dtype=tf.float32)
    x_tf = tf.convert_to_tensor(x, dtype=tf.float32)
    
    # Compute pairwise distances
    dists = tf.norm(tf.expand_dims(x_tf, 1) - tf.expand_dims(data_tf, 0), axis=2)
    
    # Sort and take k+1 nearest (excluding self)
    sorted_dists, _ = tf.nn.top_k(-dists, k=k+1)
    sorted_dists = -sorted_dists[:, 1:]  # Remove self and convert back to positive
    
    # Compute LID
    f = lambda v: - k / tf.reduce_sum(tf.math.log(v/v[-1]))
    lid = f(sorted_dists[0])
    
    return lid.numpy()

# lid of a batch of query points X
# RISK_INFO: [张量操作差异] - 该函数从 PyTorch 迁移到 TensorFlow。`torch.cdist`, `torch.sort`, `torch.unbind` 分别被 `tf.norm` 手动实现、`tf.nn.top_k`, `tf.map_fn` 替代。虽然逻辑上等价，但底层实现和数值精度可能存在差异。
def mle_batch(data, batch, k):
    ...
    dists = tf.norm(tf.expand_dims(batch_tf, 1) - tf.expand_dims(data_tf, 0), axis=2)
    
    # Sort and take k+1 nearest (excluding self)
    sorted_dists, _ = tf.nn.top_k(-dists, k=k+1)
    sorted_dists = -sorted_dists[:, 1:]  # Remove self and convert back to positive
    
    # Compute LID for each point in batch
    @tf.autograph.experimental.do_not_convert
    def compute_lid(v):
        return - k / tf.reduce_sum(tf.math.log(v/v[-1]))
    
    lids = tf.map_fn(compute_lid, sorted_dists)
...
def get_lids_random_batch(model, X, X_noisy, X_adv, dataset, k=10, batch_size=100):
    ...
    """
    # CRITICAL_ERROR: [算法流程改变] - 新架构使用`model.layers`来定义中间激活层，而原架构使用`model.children()`。PyTorch的`model.children()`返回直接子模块（可能包含多个操作），而Keras的`model.layers`返回一个扁平化的层列表。这种差异导致LID计算所依据的激活点（数量和位置）不一致。
    # 不一致的实现:
    # # In PyTorch, model.children() returns top-level modules
    # # In TensorFlow/Keras, we need to identify meaningful layer boundaries
    # # For CNN models, we typically want activations after major blocks
    # # (conv+pool blocks, dense layers, etc.)
    # 
    # # Create sub-models for extracting intermediate activations
    # layer_models = []
    # 
    # # For simplicity, we'll extract activations from all layers
    # # This might differ from PyTorch's model.children() behavior
    # # but maintains the spirit of extracting hierarchical features
    # for i in range(len(model.layers)):
    #     if i == len(model.layers) - 1:
    #         # Use the full model for the last layer
    #         layer_models.append(model)
    #     else:
    #         # Create intermediate model
    #         layer_model = tf.keras.Model(inputs=model.input, outputs=model.layers[i].output)
    #         layer_models.append(layer_model)
    # 
    # lid_dim = len(layer_models)
    ...
    #         X_act = layer_model(X[start:end], training=False)
    #         X_act = tf.reshape(X_act, (n_feed, -1))
    # 
    #         X_adv_act = layer_model(X_adv[start:end], training=False)
    #         X_adv_act = tf.reshape(X_adv_act, (n_feed, -1))
    # 
    #         X_noisy_act = layer_model(X_noisy[start:end], training=False)
    #         X_noisy_act = tf.reshape(X_noisy_act, (n_feed, -1))
    # 
    ...
    # Function to identify logical blocks similar to PyTorch's children()
    ...
    # Get logical block indices
    block_indices = get_logical_block_indices(model)
    
    # Create sub-models for extracting intermediate activations at block boundaries
    layer_models = []
    
    # Get the input layer - handle both Sequential and Functional models
    if hasattr(model, 'input'):
        model_input = model.input
    else:
        # For Sequential models, we need to access the input of the first layer
    ...
    for idx in block_indices:
        if idx == len(model.layers) - 1:
            ...
            # Create intermediate model up to this block
            layer_model = tf.keras.Model(inputs=model_input, outputs=model.layers[idx].output)
            layer_models.append(layer_model)
