...
def test(dic, X, thrs):
    ...
    pred_labels = []
    for d in dic:
        # RISK_INFO: [API 行为不等价] - tf.constant 总是复制数据，而原作中的 torch.as_tensor 在可能的情况下会避免复制（零拷贝），这可能导致内存使用上的差异。
        m = dic[d].mark(tf.constant(X))
        dist_all.append(m)
...
def main(args):
    ...
    detector_i_filename = '{}_magnet_detector_i.weights.h5'.format(args.dataset)
    detector_ii_filename = '{}_magnet_detector_ii.weights.h5'.format(args.dataset)
    ...
    datas = tf.data.Dataset.from_tensor_slices((loader_train.data, loader_train.label))
    datas = datas.shuffle(buffer_size=len(X_train)).batch(256)
    ...
    # RISK_INFO: [API 行为不等价] - tf.constant 总是复制数据，而原作中的 torch.as_tensor 在可能的情况下会避免复制（零拷贝），这可能导致内存使用上的差异。
    operator = Operator(tf.constant(x_val), tf.constant(X_test), tf.constant(Y_test), classifier.model, detector_dict, reformer)
    ...
    for attack in ATTACKS:
        ...
        # RISK_INFO: [API 行为不等价] - tf.constant 总是复制数据，而原作中的 torch.as_tensor 在可能的情况下会避免复制（零拷贝），这可能导致内存使用上的差异。
        testAttack = AttackData(tf.constant(X_test_adv), tf.constant(np.argmax(Y_test, axis=1)), attack)
