...
from art.attacks.evasion import FastGradientMethod, BasicIterativeMethod, CarliniL2Method, CarliniLInfMethod, ProjectedGradientDescent, DeepFool, SpatialTransformation, SquareAttack
# NOTE: ZooAttack and AdversarialPatchPyTorch are PyTorch-specific and not available for TensorFlow
# Will skip these attacks for TensorFlow implementation
...
def main(args):
    ...
    if args.dataset == 'mnist':
        ...
        raise NotImplementedError("Skip - ImageNet not implemented yet")
    ...
    #CWInf
    attack = CarliniLInfMethod(estimator=classifier, max_iter=20)
    adv_data = attack.generate(x=x_test)
    ...
    #CW2
    attack = CarliniL2Method(estimator=classifier, max_iter=10, confidence=10)
    adv_data = attack.generate(x=x_test)
    ...
    #DF
    attack = DeepFool(estimator=classifier)
    adv_data = attack.generate(x=x_test)
    ...
    attack = SpatialTransformation(estimator=classifier, max_translation=translation, max_rotation=rotation)
    adv_data = attack.generate(x=x_test)
    ...
    #ZOO attack
    # ZooAttack is commented out in the original implementation
    ...
    if args.dataset != 'mnist':
        # AdversarialPatchPyTorch is PyTorch-specific and not available for TensorFlow
        # Skip AP attack for TensorFlow implementation
        print('Skipping AdversarialPatch attack (PyTorch-specific)')
